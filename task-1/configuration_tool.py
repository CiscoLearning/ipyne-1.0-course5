...
def main():
    ...
    try:
        # Load inventory and test device connectivity
        inventory_data = read_inventory("inventory.csv")
        R1_data = get_device_data(inventory_data, "R1")
        
        if R1_data:
            conn_info = get_conn_info(R1_data)
            conn = connect_device(conn_info)
            if conn:
                print(f"Successfully connected to {R1_data['Name']}!")
                
                # Test the send_command function
                version_output = send_command(conn, "show version")
                print("Device version info:")
                print(version_output[:200] + "..." if len(version_output) > 200 else version_output)
